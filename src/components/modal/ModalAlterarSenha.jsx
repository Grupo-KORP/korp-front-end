...
function CampoSenha({ id, label, valor, onChange, onBlur, erro, visivel, onToggleVisivel, placeholder }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className={[
        "flex items-center gap-3 px-4 h-10 rounded-xl border bg-white transition-all duration-200",
        temErro
          ? "border-red-300 ring-1 ring-red-300"
          : "border-gray-200 focus-within:scale-[1.01]",
      ].join(" ")}
        style={!temErro ? {
          "--tw-ring-color": "rgba(10,52,144,0.25)",
        } : {}}
      >
        ...
        <input
          ...
          className="flex-1 bg-transparent outline-none text-[15px] text-gray-700 placeholder:text-gray-300 font-medium"
        />
        <button
          type="button"
    ...
    <div className="flex flex-col gap-2 px-1 mt-0.5">
      <div className="flex items-center gap-2">
        <div className="flex gap-1 flex-1">
          {[1,2,3,4].map((i) => (
            <div key={i}
              className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= pontos ? cores[pontos] : "bg-gray-200"}`}
            />
          ))}
        </div>
        {labels[pontos] && (
        ...
        {REGRAS_SENHA.map((r) => {
          ...
          return (
            <div key={r.id} className="flex items-center gap-1.5">
              <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center flex-shrink-0 transition-colors duration-200 ${ok ? "bg-emerald-500" : "bg-gray-200"}`}>
...
export default function ModalAlterarSenha({ aoConfirmar, aoFechar }) {
  ...
  const tentarFechar = () => {
    if (temPendencia) { dispararErros(); return; }
    aoFechar?.();
  };
  ...
  return createPortal(
    ...
    >
      <div
        className="w-[680px] max-w-[calc(100vw-24px)] rounded-3xl overflow-hidden flex flex-col shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── Header azul ── */}
        <div
          className="flex items-center gap-4 px-7 py-5"
          style={{ background: "#0a3490" }}
        >
          <div
            className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
            style={{ background: "rgba(255,255,255,0.12)" }}
          >
            <IcoCadeado size={20} className="text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <h2 id="modal-titulo" className="text-[17px] font-black text-white leading-tight m-0">
              Alterar Senha
            </h2>
            <p className="text-[12px] font-medium mt-0.5 mb-0 leading-snug"
              style={{ color: "rgba(255,255,255,0.55)" }}>
              Preencha todos os campos para atualizar sua senha com segurança.
            </p>
          </div>
          <div
        ...
        <div className="bg-white flex">
          ...
          <div className="flex-1 px-6 py-6 flex flex-col gap-4">
            ...
            {novaSenha ? (
              ...
              <p className="flex items-start gap-1.5 text-[12px] font-semibold text-emerald-600 m-0 pl-1">
                <IcoEscudo size={13} />
                <span><span className="font-bold">Dica:</span> use pelo menos 8 caracteres com letras, números e símbolos.</span>
              </p>
            )}
            ...
            <div className="flex gap-3 mt-auto pt-2">
              <button
                ...
                className="flex-1 h-10 rounded-xl border-2 border-gray-200 bg-white text-[13px] font-bold text-gray-600 cursor-pointer transition-all duration-150 hover:border-gray-300 hover:bg-gray-50 active:scale-[0.98]"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={!podeSalvar}
                className="flex-1 h-10 rounded-xl text-[13px] font-bold text-white transition-all duration-200 flex items-center justify-center gap-2 active:scale-[0.98]"
                style={{
                  background: podeSalvar ? "#0a3490" : "#94a3b8",
                  cursor: podeSalvar ? "pointer" : "not-allowed",
                  boxShadow: podeSalvar ? "0 6px 18px rgba(10,52,144,0.3)" : "none",
                }}
              >
                <IcoCadeado size={14} />
                Salvar senha
