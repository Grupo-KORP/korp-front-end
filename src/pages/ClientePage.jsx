...
export default function ClientePage() {
  ...
  async function handleDeleteContact() {
    ...
    try {
      // Se possui id, existe no banco
      if (contactToDelete.idContato) {
        await api.delete(`contatos/${contactToDelete.idContato}`);

        setContacts((prev) =>
          prev.filter((c) => c.idContato !== contactToDelete.idContato)
        );

        toast.success("Contato removido com sucesso!");
      } else {
        // contato criado no frontend mas ainda não salvo
  ...
  async function handleSubmit() {
    ...
    try {
      if (editingId) {
        console.log("Payload para contatos:", contatosPayload);
        // Etapa 1: atualiza dados da empresa
        await api.put(`/cliente/${editingId}`, companyPayload);
        // Etapa 2: atualiza contatos separadamente
        await api.put(`/cliente/${editingId}/contatos`, contatosPayload);
        toast.success("Cliente atualizado com sucesso!");
        setEditingId(null);
      } else {
        // Criação: payload único com contatos embutidos
        // (backend no POST ainda aceita a lista junto)
        await api.post("/cliente", { ...companyPayload, contatos: contatosPayload.contatos });
  ...
  const fields = [
    ...
    { name: "inscEst", label: "Insc. Est.", pair: "end", placeholder: "110.042.490.114", value: form.inscEst, loading: loadingCNPJ, readOnly: !!editingId },
    ...
    <>
      ...
      {contactToDelete && (
        <div
          ...
          onClick={() => !deletingContact && setContactToDelete(null)}
        >
          <div
          ...
          >
            <div className="produto-modal-header">
              <div>
                <p className="produto-eyebrow">Excluir contato</p>
                <h2>Tem certeza?</h2>
              </div>
