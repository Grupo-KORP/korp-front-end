  ...
  const TIPO_PARA_FILTRO = { liberadas: "liberada", pendentes: "pendente" };
  ...
  const normalizarPainelVendedor = (painel) => {
    ...
    return {
      ...
      vendas,
      detalhesVenda,
      paginaAtual: painel.paginaAtual || 0,
      tamanhoPagina: painel.tamanhoPagina || 5,
      totalPaginasVendas: painel.totalPaginasVendas || 0,
      totalVendasFiltradas: painel.totalVendasFiltradas || 0,
    };
  };
  ...
  ══════════════════════════════════════════ */
  const CARDS_VALIDOS = ["vendas", "liberadas", "pendentes"];

  function lerEstadoInicialDaUrl(searchParams) {
    const anoParam = parseInt(searchParams.get("ano"), 10);
    const mesParam = searchParams.get("mes");
    const diaParam = parseInt(searchParams.get("dia"), 10);
    const periodoParam = searchParams.get("periodo");
    const filtroParam = searchParams.get("filtro");
    const paginaParam = parseInt(searchParams.get("pagina"), 10);

    const mesValido = MESES.includes(mesParam) ? mesParam : MES_ATUAL;
    const tipoFiltroPeriodo = periodoParam === "month" ? "month" : "day";
    const diaSelecionado = tipoFiltroPeriodo === "day"
      ? (!isNaN(diaParam) && diaParam >= 1 && diaParam <= 31 ? diaParam : DIA_ATUAL)
      : null;

    return {
      anoSelecionado: !isNaN(anoParam) ? anoParam : ANO_ATUAL,
      mesSelecionado: mesValido,
      diaSelecionado,
      tipoFiltroPeriodo,
      cardAtivo: CARDS_VALIDOS.includes(filtroParam) ? filtroParam : null,
      paginaAtual: !isNaN(paginaParam) && paginaParam >= 0 ? paginaParam : 0,
    };
  }
  ...
  export default function HomeVendedor() {
    ...
    const estadoInicial = useRef(lerEstadoInicialDaUrl(searchParams)).current;

    const [anoSelecionado, setAnoSelecionado] = useState(estadoInicial.anoSelecionado);
    const [mesSelecionado, setMesSelecionado] = useState(estadoInicial.mesSelecionado);
    const [diaSelecionado, setDiaSelecionado] = useState(estadoInicial.diaSelecionado);
    const [tipoFiltroPeriodo, setTipoFiltroPeriodo] = useState(estadoInicial.tipoFiltroPeriodo);
    const [mostrarMeses, setMostrarMeses] = useState(false);
    const [selecao, setSelecao] = useState(() => {
      const mesIndex = MESES.indexOf(estadoInicial.mesSelecionado);
      if (estadoInicial.tipoFiltroPeriodo === "day") {
        return { type: "day", y: estadoInicial.anoSelecionado, m: mesIndex, d: estadoInicial.diaSelecionado };
      }
      return { type: "month", y: estadoInicial.anoSelecionado, m: mesIndex };
    });
    const [cardAtivo, setCardAtivo] = useState(estadoInicial.cardAtivo);
    ...
    useEffect(() => {
      ...
      const filtroPeriodo = {
        ...
        tipo: TIPO_PARA_FILTRO[cardAtivo] || null,
        pagina: paginaAtual,
    ...
    }, [diaSelecionado, mesSelecionado, anoSelecionado, tipoFiltroPeriodo, atualizacaoPainel, cardAtivo, paginaAtual, tamanhoPagina]);

    // ── Mantém a URL sincronizada com o filtro/página atuais ──
    useEffect(() => {
      const params = {};
      if (anoSelecionado !== ANO_ATUAL) params.ano = String(anoSelecionado);
      if (mesSelecionado !== MES_ATUAL) params.mes = mesSelecionado;
      if (tipoFiltroPeriodo === "month") {
        params.periodo = "month";
      } else if (diaSelecionado !== DIA_ATUAL) {
        params.dia = String(diaSelecionado);
      }
      if (cardAtivo) params.filtro = cardAtivo;
      if (paginaAtual > 0) params.pagina = String(paginaAtual);

      setSearchParams(params, { replace: true });
    }, [anoSelecionado, mesSelecionado, diaSelecionado, tipoFiltroPeriodo, cardAtivo, paginaAtual, setSearchParams]);
    ...
    const vendasExibidas = dados.vendas;
    ...
    /**
     * Busca TODAS as vendas do período/filtro atual (sem paginação) para
     * exportação em PDF, sem afetar o estado paginado exibido na tela.
     */
    async function buscarDadosCompletosParaPdf() {
      const mes = MESES.indexOf(mesSelecionado) + 1;
      ...
      const response = await buscarPainelVendedor(filtroCompleto);
      return normalizarPainelVendedor(response) || dados;
    }

    async function gerarPDF() {
      setGerandoPdf(true);
      try {
        const dadosPdf = await buscarDadosCompletosParaPdf();
        ...
        const pdfHtml = `
          <html>
            ...
            <body>
              ...
              <div class="resumo">
                <div class="caixa"><div class="rotulo">Total de Vendas</div><div class="valor">${dadosPdf.totalVendas}</div></div>
                <div class="caixa"><div class="rotulo">Comissões Liberadas</div><div class="valor">${dadosPdf.comissoesLiberadas}</div></div>
                <div class="caixa"><div class="rotulo">Pagamentos Pendentes</div><div class="valor">${dadosPdf.pagamentosPendentes}</div></div>
                <div class="caixa"><div class="rotulo">Projeção Bruta</div><div class="valor">${dadosPdf.projecao}</div></div>
              </div>
              <table>
                <thead>
    ...
    async function gerarPDFRelatorio() {
      ...
      try {
        ...
        const metricCards = () => {
          ...
          const metrics = [
            { label: "Total de Vendas", value: dadosPdf.totalVendas, color: [30, 41, 59] },
            { label: "Comissoes Liberadas", value: dadosPdf.comissoesLiberadas, color: [22, 163, 74] },
            { label: "Pagamentos Pendentes", value: dadosPdf.pagamentosPendentes, color: [217, 119, 6] },
            { label: "Projecao Bruta", value: dadosPdf.projecao, color: [30, 58, 138] },
          ];
          ensureSpace(30);
            ...
            <div className="flex flex-col lg:flex-row gap-5 lg:items-stretch lg:flex-1 lg:min-h-0">
              ...
              <div className="flex flex-col gap-4 lg:flex-1 lg:min-h-0">
                ...
                <div className={`${cardBg} rounded-2xl shadow-sm p-5 flex flex-col lg:flex-1 lg:min-h-0`}>
                  ...
                  {/* Linhas */}
                  <div className="flex flex-col gap-0.5 lg:flex-1 overflow-y-auto max-h-72 lg:max-h-none pr-2 lg:min-h-0" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(156,163,175,0.35) transparent" }}>
                  ...
                  {/* Paginação */}
                  {dados.totalPaginasVendas > 1 && (
                    <div className={`flex items-center justify-between pt-3 mt-2 border-t ${borda}`}>
                      <span className={`text-[10px] ${textoS}`}>
                        Pág. {dados.paginaAtual + 1} de {dados.totalPaginasVendas} • {dados.totalVendasFiltradas} vendas
                      </span>
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => setPaginaAtual(0)}
                          disabled={dados.paginaAtual === 0}
                          className={`px-2 py-1 rounded-lg text-xs font-semibold border ${borda} ${textoM} disabled:opacity-40 ${hover}`}
                        ...
                        <button
                          ...
                          disabled={dados.paginaAtual === 0}
                          className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${borda} ${textoM} disabled:opacity-40 ${hover}`}
                        >
                          Anterior
                        </button>
                        <button
                          onClick={() => setPaginaAtual((p) => Math.min(p + 1, dados.totalPaginasVendas - 1))}
                          disabled={dados.paginaAtual >= dados.totalPaginasVendas - 1}
                          className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${borda} ${textoM} disabled:opacity-40 ${hover}`}
                        >
                          Próxima
                        </button>
                        <button
                          onClick={() => setPaginaAtual(dados.totalPaginasVendas - 1)}
                          disabled={dados.paginaAtual >= dados.totalPaginasVendas - 1}
                          className={`px-2 py-1 rounded-lg text-xs font-semibold border ${borda} ${textoM} disabled:opacity-40 ${hover}`}
                          title="Última página"
                        >
              ...
              <div className="w-full lg:w-64 flex-shrink-0 flex flex-col gap-3 lg:min-h-0">
                ...
                <div className={`${cardBg} rounded-2xl shadow-sm p-4 flex flex-col gap-2 lg:flex-1 lg:min-h-0`}>
                  ...
                  <div className="flex flex-col gap-1.5 overflow-y-auto max-h-48 lg:max-h-none lg:flex-1 lg:min-h-0 pr-1" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(156,163,175,0.35) transparent" }}>
                ...
                <button
                  ...
                  disabled={gerandoPdf}
                  className="w-full py-2.5 rounded-xl text-sm font-bold text-white tracking-wide transition-all duration-200 hover:opacity-90 active:scale-[0.98] shadow-md disabled:opacity-60"
